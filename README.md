...
# ============================================================
# NEW: BUSINESS LINE → PRODUCT (LAST PARAGRAPH)
# ============================================================

def bizline_with_product(df_cy, df_py, region_col, region,
                         biz_col, product_col, metric_col):
    ...
    def describe_biz(r):
        biz = r[biz_col]
        base = f"{biz} ({fmt_change_yoy(r['Change'], r['YoY%'], metric_col)})"

        prod_agg = drop_noise(
            compute_agg(
                df_cy[(df_cy[region_col] == region) & (df_cy[biz_col] == biz)],
                df_py[(df_py[region_col] == region) & (df_py[biz_col] == biz)],
                [product_col], metric_col
            )
        )

        if r["Change"] > 0:
            prod = select_by_coverage(
                prod_agg[prod_agg["Change"] > 0].sort_values("Change", ascending=False),
                metric_col
            )
        else:
            prod = select_by_coverage(
                prod_agg[prod_agg["Change"] < 0].sort_values("Change"),
                metric_col
            )

        return f"{base} driven by {join_items(prod, product_col, metric_col)}" if not prod.empty else base

    main = " and ".join(describe_biz(r) for _, r in top.iterrows())

    text = f"Products - In the {region} region, growth was led by {main}"

    if not bottom.empty:
        offset = " and ".join(describe_biz(r) for _, r in bottom.iterrows())
        text += f", partially offset by {offset}."
...
def region_commentary(df_cy, df_py, region_col, region, metric_col,
                      segment_col, country_col, biz_col, product_col):
    ...
    return "\n".join([
        ...
        bizline_with_product(df_cy, df_py, region_col, region, biz_col, product_col, metric_col)
    ])
...
# ============================================================
# WORD WRITER (NUMBERS + PERCENTAGES COLORED)
# ============================================================
