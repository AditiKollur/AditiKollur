...
# Headers with 3 empty columns after every 2 columns
...
for _, rep in list_rep.iterrows():
    ...
    ead_col = rep["EAD"]   # column letter
    rwa_col = rep["RWA"]   # column letter

    used = ws.used_range
    last_row = used.last_cell.row

    # ---- BULK READ COLUMNS (CRITICAL FOR SPEED) ----
    ead_values = ws.range(f"{ead_col}1:{ead_col}{last_row}").value
    rwa_values = ws.range(f"{rwa_col}1:{rwa_col}{last_row}").value

    if not ead_values or not rwa_values:
        continue

    # ---- FIND HEADER ROW ----
    start_row = None
    for idx, (e, r) in enumerate(zip(ead_values, rwa_values)):
        if (
    ...
    # ---- NUMERIC SCAN (PURE PYTHON) ----
    ead_nums = []
    rwa_nums = []
    ead_nonzero = None
    rwa_nonzero = None

    for i in range(start_row - 1, len(ead_values)):
        e = ead_values[i]
        r = rwa_values[i]
...
print("Completed successfully with optimized performance.")
